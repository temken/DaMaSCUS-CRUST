...
double Cutoff_Speed(const std::string& experiment, double mDM, int rank)
{
	...
	{
		if(rank == 0)
			cout << "Warning: vCutoff>vEsc+vEarth for mDM = " << mDM << "GeV." << endl;
		vcut = -1;
	}
	return vcut;
}
...
void Propagate(Event& event, double& weight, std::mt19937& PRNG, double logXi = -1.0)
{
	...
	double t_exit;
	if(next_layer == 0)
		t_exit = -event.Position()[2] / vz;
	else if(next_layer == Detector_Index)
		t_exit = (-Detector_Depth - event.Position()[2]) / vz;
	else if(vz > 0)
		t_exit = (-Layers[layer_old - 1].depth - event.Position()[2]) / vz;
	else
		t_exit = (-Layers[next_layer - 1].depth - event.Position()[2]) / vz;
	double d = t_exit * v;
	// Mean free path
	...
	else
	{
		...
		event.IncreaseTime(dt);
		event.IncreasePosition(dt * event.Velocity());
		if(Current_Layer(event) == layer_old)
			cout << "Transition from " << layer_old << " to " << layer_old << " at z=" << event.Position()[2] / meter << "m." << endl;
		logXi -= d / mfp;
...
double Sample_ScatteringAngle(const DM_Particle& DM, const std::vector<double>& target, double vDM, double delta, double& weight, std::mt19937& PRNG)
{
	weight			= 1.0;
	double cosalpha = 0.0;
	if(delta < 1e-6 || PDF_ScatterAngle(-1.0, DM, target, vDM, delta) < 0.0)
		delta = 0;
